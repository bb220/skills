...
description: Postman collection specification details for API documentation and testing. Use when generating a Postman collection from an API's source code.
license: MIT
metadata: 
...
## Quick Start
Evaluate the API source code to extract endpoint information, request/response examples, and authentication details. Use this information to construct a JSON object that adheres to the Postman collection schema. This includes defining the collection's `info`, `item` (endpoints), `event` (scripts), and `auth` (authentication) properties.

## Postman collection schema
The Postman collection schema is a JSON structure that defines the organization and content of a Postman collection.
